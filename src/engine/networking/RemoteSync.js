 ...
 * @param {NetworkClient} client - NetworkClient handling data transfer
 */
class RemoteSync {
  constructor(client) {
    this.client = client;
   ...
   * @param {Object3D} object - for 'update'
   * @param {function} func - callback function
   */

  addEventListener(type, arg1, arg2) {
    var func, object;
    ...
    switch (type) {
      ...
      default:
        console.log("RemoteSync.addEventListener: Unknown type " + type);
        break;
    }
  }
  ...
  addLocalObject(object, info, recursive) {
    if (this.localObjectTable[object.uuid] !== undefined) {
      console.warn(
        "RemoteSync.addLocalObject: This object has already been registered."
      );
      return;
    }
  ...
  removeLocalObject(object) {
    ...
    if (this.client.connectionNum() > 0)
      this.broadcastRemoveObjectRequest(object);
  }
  ...
  addRemoteObject(destId, objectId, object) {
    ...
    if (objects === undefined) {
      console.warn(
        "RemoteSync.addRemoteObject: has not received any add object request from " +
          destId +
          " peer."
      );
      return;
    ...
    if (info === undefined) {
      console.warn(
        "RemoteSync.addRemoteObject: has not received " +
          objectId +
          " object addition request from " +
          destId +
          " peer."
      );
      return;
    ...
    if (objects[objectId] !== undefined) {
      console.warn(
        "RemoteSync.addRemoteObject: object for " +
          objectId +
          " object of " +
          destId +
          " peer has been already registered."
      );
      return;
    }
    ...
    if (info.recursive === true) {
      function traverse(obj, param) {
        ...
        for (
          var i = 0, il = Math.min(children1.length, children2.length);
          i < il;
          i++
        ) {
  ...
  addSharedObject(object, id, recursive) {
    ...
    if (this.sharedObjectTable[id] !== undefined) {
      console.warn(
        "RemoteSystem.addSharedObject: Shared id " + id + " is already used."
      );
      return;
    }
    ...
    this.client.addEventListener(
      ...
      function (component) {
        ...
        switch (component.type) {
          ...
          default:
            console.log("RemoteSync: Unknown type " + component.type);
            break;
        }
   ...
   * @param {Object3D} object - object will be updated with component
  ...
  deserialize(object, component) {
    ...
    if (
      ...
      component.morphTargetInfluences.length > 0
    ) {
      var array = component.morphTargetInfluences;
      var array2 = object.morphTargetInfluences;
  ...
  broadcastAddObjectRequest(object) {
    this.client.broadcast(
      buildObjectAdditionComponent(
        this.id,
        object,
        this.localObjectInfos[object.uuid]
      )
    );
  }
   ...
   * @param {string} destId - remote peer id
   */
  sendAddObjectsRequest(destId) {
    var component = buildObjectsAdditionComponent(
      this.id,
      this.localObjects,
   ...
   * @param {Object3D} object
   */
  broadcastRemoveObjectRequest(object) {
  ...
  invokeConnectListeners(id, fromRemote) {
  ...
  invokeReceiveUserDataListeners(component) {
  ...
  invokeMasterListeners() {
    for (var i = 0, il = this.onMasters.length; i < il; i++) {
      this.onMasters[i]();
    }
  }

  invokeSlaveListeners() {
    for (var i = 0, il = this.onSlaves.length; i < il; i++) {
      this.onSlaves[i]();
    }
  }

  invokeMasterNotificationListeners(masterPeer) {
 ...
 * @param {Array} objects - Array of Object3D
...
class NetworkClient {
  constructor(params) {
    ...
    if (params.onOpen !== undefined)
      this.addEventListener("open", params.onOpen);
    if (params.onClose !== undefined)
      this.addEventListener("close", params.onClose);
    if (params.onError !== undefined)
      this.addEventListener("error", params.onError);
    if (params.onConnect !== undefined)
      this.addEventListener("connect", params.onConnect);
    if (params.onDisconnect !== undefined)
      this.addEventListener("disconnect", params.onDisconnect);
    if (params.onReceive !== undefined)
      this.addEventListener("receive", params.onReceive);
    if (params.onRemoteStream !== undefined)
      this.addEventListener("remote_stream", params.onRemoteStream);
  }
